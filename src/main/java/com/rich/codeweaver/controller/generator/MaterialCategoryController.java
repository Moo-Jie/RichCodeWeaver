...
import com.rich.codeweaver.model.dto.materialcategory.MaterialCategoryAddRequest;
import com.rich.codeweaver.model.dto.materialcategory.MaterialCategoryQueryRequest;
import com.rich.codeweaver.model.dto.materialcategory.MaterialCategoryUpdateRequest;
import com.rich.codeweaver.model.entity.MaterialCategory;
import com.rich.codeweaver.model.entity.User;
import com.rich.codeweaver.model.vo.MaterialCategoryVO;
import com.rich.codeweaver.service.social.MaterialCategoryService;
...
public class MaterialCategoryController {
    ...
    public BaseResponse<Long> addMaterialCategory(@RequestBody MaterialCategoryAddRequest addRequest,
        ...
        ThrowUtils.throwIf(!userService.isAdmin(loginUser), ErrorCode.NO_AUTH_ERROR, "仅管理员可添加分类");
    ...
    public BaseResponse<Boolean> updateMaterialCategory(@RequestBody MaterialCategoryUpdateRequest updateRequest,
        ...
        // 权限校验：仅管理员可操作
        User loginUser = userService.getLoginUser(request);
        ThrowUtils.throwIf(!userService.isAdmin(loginUser), ErrorCode.NO_AUTH_ERROR, "仅管理员可修改分类");
    ...
    public BaseResponse<Boolean> deleteMaterialCategory(@RequestBody DeleteRequest deleteRequest,
        ...
        // 权限校验：仅管理员可操作
        User loginUser = userService.getLoginUser(request);
        ThrowUtils.throwIf(!userService.isAdmin(loginUser), ErrorCode.NO_AUTH_ERROR, "仅管理员可删除分类");
    ...
    public BaseResponse<Page<MaterialCategoryVO>> listMaterialCategoryByPage(
        ...
        // 权限校验：仅管理员可操作
        User loginUser = userService.getLoginUser(request);
        ThrowUtils.throwIf(!userService.isAdmin(loginUser), ErrorCode.NO_AUTH_ERROR, "仅管理员可查看分类列表");
